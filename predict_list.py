...
class PPIDataset(Dataset):
    def __init__(self, ppi_df, device) -> None:
        super().__init__()
        self.ppi_df = ppi_df
...
class CustomWriter(BasePredictionWriter):
    def __init__(self, output_dir, write_interval,ppi_df):
    ...
    def write_on_batch_end(self, trainer, pl_module, prediction, batch_indices, batch, batch_idx, dataloader_idx):
        protein_pair = str(self.ppi_df.loc[dataloader_idx][0])
        save_dir = os.path.join(self.output_dir,protein_pair)
        ...
        np.savetxt(os.path.join(save_dir, f"pred_interaction.txt"),pred_interaction)
        np.savetxt(os.path.join(save_dir, f"pred_contact.txt"), contact_map.squeeze())


if __name__ == "__main__":

    ppi_list, result_path, model_ckpt, device = sys.argv[1:]
    ppi_df = pd.read_csv(ppi_list)
    ...
    dataset = PPIDataset(ppi_df,result_path, device)
    dataloader = DataLoader(dataset, batch_size=1)
    pred_writer = CustomWriter(output_dir=result_path, write_interval="batch",ppi_df=ppi_df)
