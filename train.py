...
class PPIDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        interaction = self.ppi_df.loc[index][-1]
        len1,len2 = ppi_df.loc[index][1:3]
        if interaction:
            contact_map = np.loadtxt(ppi_df.loc[index][-1])
        else:
            contact_map = np.zeros(shape=(len1, len2))
        ...
        return (nodes1, edges1, edge_index1, nodes2, edges2, edge_index2, seq1, seq2),(contact_map,interaction)
...
if __name__ == "__main__":
    ...
    ppi_list, result_path, device ,contact= sys.argv[1:]
    ...
    os.makedirs(os.path.join(result_path,'checkpoints'))
    checkpoint_callback = ModelCheckpoint(
        dirpath=os.path.join(result_path,'checkpoints'),
        filename="model {epoch:d}",
        every_n_epochs=1,
        monitor='val_loss',
        save_top_k=-1
    )
    csv_logger = CSVLogger("logs", name="PLMDA-PPI")
    trainer = pl.Trainer(
        callbacks=[checkpoint_callback],
        # devices=hyper_params["devices"],
        max_epochs=hyper_params["epochs"],
        min_epochs=hyper_params["epochs"],
        accelerator = "cpu",
